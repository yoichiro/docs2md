...
const inlineImage = async (
  ...
  const inlineObject =
    document.data.inlineObjects[element.inlineObjectElement.inlineObjectId];
  try {
...
const getDocumentIdFromDocUrl = (source) => {
  const regexp =
    /https:\/\/docs\.google\.com\/document\/d\/([0-9a-zA-Z_]+)\/edit/;
...
const main = async () => {
  ...
  let documentId = await getDataFromPrompt(
    "documentId",
    "Document ID",
  ...
  const imageFilenamePrefix = await getDataFromPrompt(
    "imageFilenamePrefix",
    "Image Filename Prefix",
    previousData.imageFilenamePrefix
  );
  ...
  savePreviousData(previousData);
  const markdownFilename = await getDataFromPrompt(
    "markdownFilename",
    "Markdown filename",
    previousData.markdownFilename
  );
  if (!markdownFilename) {
    return;
  }
  ...
  for (let items of document.data.body.content) {
    if (items.paragraph) {
      if (items.paragraph.paragraphStyle.namedStyleType === "NORMAL_TEXT") {
        ...
        items.paragraph.paragraphStyle.namedStyleType === "HEADING_1"
      ) {
        const heads = items.paragraph.elements
          .map((element) => {
            if (element.textRun) {
        ...
        items.paragraph.paragraphStyle.namedStyleType === "HEADING_2"
      ) {
        const heads = items.paragraph.elements
          .map((element) => {
            if (element.textRun) {
      ...
      const code = [];
      items.table.tableRows[0].tableCells[0].content.forEach((c) => {
